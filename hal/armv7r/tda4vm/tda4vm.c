...
static const unsigned char tda4vm_deskew_pll_divide_to_val[5] = { 0b11, 0b10, 0b01, 0b11, 0b00 };
...
int tda4vm_setPLL(unsigned pll, const tda4vm_clk_pll_t *config, const unsigned *hsdivs, unsigned int n_hsdivs)
{
	...
	if ((hw_config & 0x3) == 2) {
		...
		*(base + pll_reg_lockkey1) = PLL_LOCKKEY_1;
		/* TODO: actually configure this type of PLL */
		(void)tda4vm_deskew_pll_divide_to_val;
		/* Lock configuration again */
		*(base + pll_reg_lockkey0) = 0;
		*(base + pll_reg_lockkey1) = 0;
	}
	else {
		/* TODO: special treatment necessary for PLL0 and MCU_PLL0 */
		/* Fractional PLL */
...
#if 0
		{
			.base = MAIN_UART0_BASE_ADDR,
			.clksel = -1, /* No CLKSEL for this UART */
			.clkdiv = clkdiv_usart0,
			.divisor = 1,
			.pll = clk_main_per0_pll1,
			.hsdiv = 0,
			.irq = MAIN_UART_ROUTED_IRQ,
			.pins = {
				{ pin_uart0_txd, 0, 1 },
				{ pin_prg1_pru0_gpo1, 14, 1 },
				{ pin_uart0_rxd, 0, 0 },
				{ pin_prg1_pru0_gpo0, 14, 0 },
				{ -1, 0, 0 },
			},
		},
#endif
