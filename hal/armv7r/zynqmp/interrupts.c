 ...
 *
 * GIC compatible interrupt controller driver
 *
...
int hal_interruptsSet(unsigned int n, int (*f)(unsigned int, void *), void *data)
{
	...
	if (f == NULL) {
		...
		interrupts_setCPU(n, DEFAULT_MASK);          /* CPU 0 handle all irqs                  */
...
void interrupts_init(void)
{
	...
	*(interrupts_common.gic + gicd_ctlr) &= ~0x3;
	...
	for (i = SPI_FIRST_IRQID; i < SIZE_INTERRUPTS; ++i) {
		...
		interrupts_setCPU(i, DEFAULT_MASK);
	}
	...
	*(interrupts_common.gic + gicc_pmr) = 0xff;
	...
	/* EnableGrp0 = 1; EnableGrp1 = 1; AckCtl = 1 */
	*(interrupts_common.gic + gicc_ctlr) = *(interrupts_common.gic + gicc_ctlr) | 0x7;
