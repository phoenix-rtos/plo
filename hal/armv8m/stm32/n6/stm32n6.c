 ...
 *      MSI ─┤U├─ PLL2 ─┤M├─ IC2  → SYSB_CLK (for internal busses)
...
static void _stm32_configureClocks(void)
{
	...
	static const struct {
		...
		{ .iclk = 2, .pll = 1, .div = 3 },  /* 1200 / 3 => 400 MHz (busses) */
		{ .iclk = 3, .pll = 1, .div = 3 },  /* 1200 / 3 => 400 MHz (XSPI Flash) */
		{ .iclk = 5, .pll = 4, .div = 8 },  /* 800 / 8  => 100 MHz (peripherals) */
	...
	*(stm32_common.rcc + rcc_hsecfgr) = v;
	...
	v &= ~(1 << 6); /* HSEDIV2SEL off */

	*(stm32_common.rcc + rcc_csr) = (1 << 4);
	...
	*(stm32_common.rcc + rcc_cfgr1) = v;
	...
	/* Set IC5 as clock source for PER (common peripheral) clock */
	_stm32_rccSetIPClk(ipclk_persel, ipclk_persel_ic5_ck);
...
static void _stm32_initPowerSupply(unsigned int supply, int is1v8)
{
	volatile int i;
...
void _stm32_init(void)
{
	...
	*(stm32_common.rcc + rcc_rsr) = 1 << 16;
	...
	_stm32_initPowerSupply(pwr_supply_vddio3, 1);
