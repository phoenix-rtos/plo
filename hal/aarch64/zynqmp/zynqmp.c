...
#define GPU_BASE_ADDRESS        0xff0a0000
...
void reset_pl_via_emio_pin(void)
{
...
{
	/* Both AXI interfaces in Full Power Domain configured to 32-bit width */
	*(zynq_common.fpd_slcr + fpd_slcr_afi_fs) = (FPD_AXI_S0_WIDTH << 8)
		| (FPD_AXI_S1_WIDTH << 10);

	/* AXI interface in Low Power Domain configured to 32-bit width */
...
void _zynqmp_init(void)
{
	...
	zynq_common.gpio = (void *)GPU_BASE_ADDRESS;
