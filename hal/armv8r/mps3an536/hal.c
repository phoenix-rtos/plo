 ...
 *
 * Hardware Abstraction Layer (ARMv8 Cortex-R)
 *
...
int hal_cpuJump(void)
{
	...
	hal_dcacheFlush((addr_t)ADDR_BRAM, (addr_t)ADDR_BRAM + SIZE_BRAM);

	/* Not necessary to flush entire 3GB DDR, 512K is more than enough */
	hal_dcacheFlush((addr_t)ADDR_DDR, (addr_t)ADDR_DDR + (512 * 1024));
