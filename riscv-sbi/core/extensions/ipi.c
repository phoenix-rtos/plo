...
static void _sbi_ipiListAppend(u32 hartid, ipi_node_t *node)
{
	ipi_node_t *tail;

	if (ipi_common.tasks[hartid] == NULL) {
		ipi_common.tasks[hartid] = node;
		return;
	}

	tail = ipi_common.tasks[hartid];
...
static ipi_node_t *_sbi_ipiListPop(u32 hartid)
{
	ipi_node_t *node;
	...
	ipi_common.tasks[hartid] = node->next;
	node->next = NULL;
...
long sbi_ipiSend(u32 hartid, void (*handler)(void *), void *data)
{
	...
	_sbi_ipiListAppend(hartid, node);
