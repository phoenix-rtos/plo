...
static ssize_t flashdrv_write(unsigned int dn, addr_t offs, const u8 *buff, unsigned int len)
{
	...
	while (size) {
		if (size < 0x100) {
			...
			hal_memset(flashdrv_common.buff + size, 0xff, 0x100 - size);
			if (flashdrv_bufferedPagesWrite(ctx, offs + bOffs, (const char *)flashdrv_common.buff, 0x100) < 0)
				return ERR_ARG;
			size = 0;
			break;
		}

		if (flashdrv_bufferedPagesWrite(ctx, offs + bOffs, (const char *)(buff + bOffs), 0x100) < 0)
			return ERR_ARG;
