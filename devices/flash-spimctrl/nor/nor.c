...
static const struct nor_info flashInfo[] = {
	/* Macronix (MXIX) */
	{ FLASH_ID(VID_MACRONIX, 0x2019u), "MX25L25635F", 32 * 1024 * 1024, 0x100, 0x1000, 2, 120, 150 * 1000 },
	/* Spansion */
	{ FLASH_ID(VID_SPANSION, 0x2018u), "S25FL128S", 16 * 1024 * 1024, 0x100, 0x10000, 1, 650, 165 * 1000 }
};
...
int nor_eraseSector(spimctrl_t *spimctrl, addr_t addr, time_t timeout)
{
	...
	const u8 cmd[4] = { spimctrl->dev.cmds->se, (eraseAddr >> 16) & 0xff, (eraseAddr >> 8) & 0xff, eraseAddr & 0xff };

	res = nor_validateEar(spimctrl, eraseAddr);
	if (res < EOK) {
		return res;
	}
...
int nor_pageProgram(spimctrl_t *spimctrl, addr_t addr, const void *src, size_t len, time_t timeout)
{
	...
	const u8 cmd[4] = { spimctrl->dev.cmds->pp, (programAddr >> 16) & 0xff, (programAddr >> 8) & 0xff, programAddr & 0xff };

	int res = nor_validateEar(spimctrl, programAddr);
	if (res < EOK) {
		return res;
	}
...
{
	addr_t realAddr = addr + spimctrl->moffs;
	if (((realAddr & 0xff000000) == 0) && (((realAddr + size) & 0xff000000) != 0)) {
		/* If we'd have to change EAR register during read,
		 * read data through command (can be read without EAR change)
		 */
		return nor_readCmd(spimctrl, realAddr, data, size);
	}
	else {
		/* Direct copy - offset is added by the core */
