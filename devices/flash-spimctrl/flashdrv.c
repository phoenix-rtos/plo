...
static const struct {
	...
	{ SPIMCTRL0_BASE, FLASH0_ADDR, FLASH0_OFFSET },
};
...
static int flashdrv_sync(unsigned int minor)
{
	...
	return fdrv_directSectorWrite(spimctrl, spimctrl->dev.sectorBufAddr, spimctrl->dev.sectorBuf);
}
...
static int flashdrv_init(unsigned int minor)
{
	...
	spimctrl->moffs = fdrv_info[minor].moffs;
