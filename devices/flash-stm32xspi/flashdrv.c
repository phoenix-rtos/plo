...
static u32 flashdrv_changeCtrlMode(unsigned int minor, u32 new_mode)
{
	...
	if (prev_mode == XSPI_CR_MODE_MEMORY) {
		hal_cpuDataMemoryBarrier();
		*(p->ctrl + xspi_cr) = v | (1 << 1); /* Abort memory-mapped read */
...
static void flashdrv_initPins(const struct flash_ctrlParams *p)
{
	unsigned int i;
	for (i = 0; i < MAX_PINS; i++) {
		...
		_stm32_gpioConfig(p->pins[i].port, p->pins[i].pin, gpio_mode_af, p->pin_af, gpio_otype_pp, gpio_ospeed_vhi, gpio_pupd_nopull);
	}
}
...
static ssize_t flashdrv_write_internal(unsigned int minor, addr_t offs, const void *buff, size_t len)
{
	...
	u32 page_remaining, unaligned_end = 0;
	ssize_t ret;
	u8 tmp[2];
	...
	unaligned_end = remaining % 2;
	remaining -= unaligned_end;

	while (remaining > 0) {
		page_remaining = 256 - (offs % 256);
		if (page_remaining > remaining) {
			page_remaining = remaining;
		}

		opDef.addr = offs;
		opDef.dataLen = page_remaining;
		ret = flashdrv_performWriteOp(minor, &opDef, buff, 1000);
		if (ret < 0) {
			return ret;
		}
...
static int flashdrv_init(unsigned int minor)
{
	...
	*(p->ctrl + xspi_dcr2) =
			(0 << 16) |                /* Wrapped reads not supported */
			((p->divider - 1) & 0xff); /* Clock prescaler */
