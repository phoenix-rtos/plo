...
static void mce_storeKey(mce_t per, u32 reg, u8 *key, u32 keysize)
{
	size_t i;
...
int mce_configureRegion(mce_t per, mce_reg_t reg, addr_t start, addr_t end, u32 cipher, u32 mode, u8 *key)
{
	...
	ret = mce_configureRegionInternal(per, reg, mode, ctxid, s->region.base + start, s->region.base + end - 1);
	if (ret < 0) {
		return ret;
	}
