...
static struct {
	...
	u8 regions[XSPI_N_CONTROLLERS]; /* Allocated region count */
	xspi_memregion_t region[XSPI_N_CONTROLLERS][XSPI_MCE_REGIONS];
} xspi_common = {
	.xspiDone = {},
	.regions = {},
	.region = {},
...
static int xspidrv_isValidAddress(unsigned int minor, u32 off, size_t size)
{
	...
	for (int i = 0; i < XSPI_MCE_REGIONS; i++) {
		if (xspi_common.region[minor][i].encrypt == 0) {
			continue;
		}
		/* Range overlaps */
		if ((xspi_common.region[minor][i].start <= off) && (end >= xspi_common.region[minor][i].start)) {
			return 0;
		}
		else if ((off < xspi_common.region[minor][i].end) && (end > xspi_common.region[minor][i].start)) {
			return 0;
		}
	}
...
static int xspidrv_cryp_isValidAddress(unsigned int minor, u32 off, size_t size)
{
	size_t fsize = xspi_memSize[minor];
	size_t end = off + size;
	if (end < off) {
	...
	/* Note: shouldn't this be ((off + size) < fsize) ?  */
	if (!((off < fsize) && ((off + size) <= fsize))) {
		return 0;
	}
	...
	for (int i = 0; i < mce_regcount; i++) {
		...
		/* Range inside region */
		if ((xspi_common.region[minor][i].start >= off) && (end <= xspi_common.region[minor][i].end)) {
			return 1;
		}
	}
...
static int xspidrv_cryp_sync(unsigned int minor)
{
	if (xspidrv_isValidMinor(minor) == 0) {
		return -EINVAL;
	}
...
static ssize_t xspidrv_cryp_read(unsigned int minor, addr_t offs, void *buff, size_t len, time_t timeout)
{
	if ((xspidrv_isValidMinor(minor) == 0) || (xspidrv_cryp_isValidAddress(minor, offs, len) == 0)) {
		return -EINVAL;
	}
...
static ssize_t xspidrv_cryp_write(unsigned int minor, addr_t offs, const void *buff, size_t len)
{
	if ((xspidrv_isValidMinor(minor) == 0) || (xspidrv_cryp_isValidAddress(minor, offs, len) == 0)) {
		return -EINVAL;
	}
	...
	if (xspi_ctrlParams[minor].isHyperbus != 0) {
		...
		return xspi_regcom_write_mapped(minor, offs, buff, len);
	}
}
...
static ssize_t xspidrv_cryp_erase(unsigned int minor, addr_t offs, size_t len, unsigned int flags)
{
	if (xspidrv_isValidMinor(minor) == 0) {
		return -EINVAL;
	}

	if ((len != (size_t)-1) && (xspidrv_cryp_isValidAddress(minor, offs, len) == 0)) {
		return -EINVAL;
	}
...
static int xspidrv_cryp_map(unsigned int minor, addr_t addr, size_t sz, int mode, addr_t memaddr, size_t memsz, int memmode, addr_t *a)
{
	size_t ctrlSz;
	addr_t fStart;
	...
	*a = fStart;
	...
	/* Check if region is located on device */
	if (xspidrv_cryp_isValidAddress(minor, addr, sz) == 0) {
		return -EINVAL;
	}
...
static int xspidrv_cryp_control(unsigned int minor, int cmd, void *args)
{
	const xspi_ctrlParams_t *p;
	const xspi_memcrypt_args_t *mce_args;
	mce_reg_t reg;
	int ret;
	u32 size;

	ret = xspidrv_isValidMinor(minor);
	if (ret < 0) {
		return -EINVAL;
	}
	p = &xspi_ctrlParams[minor];
	if (cmd != DEV_CONTROL_MEMCRYPT) {
		return -EINVAL;
	}
	if (args == NULL) {
		return -EINVAL;
	}
	mce_args = (const xspi_memcrypt_args_t *)args;
	if (xspi_common.regions[minor] >= XSPI_MCE_REGIONS) {
		return -EINVAL;
	}
	...
	/* Check if the region is currently non encrypted memory. Note: Is it a bad idea depend on a xspidrv_ function? */
	ret = xspidrv_isValidAddress(minor, mce_args->start, size);
	if (ret < 0) {
		return -EINVAL;
	}

	reg = xspi_common.regions[minor];

	ret = mce_configureRegion(p->mceDev, reg, mce_args->start, mce_args->end, mce_args->cipher, mce_args->mode, mce_args->key);
	if (ret < 0) {
		return -EINVAL;
	}

	/* Remember that region is configured */
	xspi_common.region[minor][reg].start = mce_args->start;
	xspi_common.region[minor][reg].end = mce_args->end;
	xspi_common.region[minor][reg].encrypt = 1;
	xspi_common.regions[minor]++;
...
__attribute__((constructor)) static void xspi_common_reg(void)
{
	...
	static const dev_ops_t opsCrypStorageSTM32_XSPI = {
		.sync = xspidrv_cryp_sync,
		.map = xspidrv_cryp_map,
		.control = xspidrv_cryp_control,
		.read = xspidrv_cryp_read,
		.write = xspidrv_cryp_write,
		.erase = xspidrv_cryp_erase,
	};

	static const dev_t devCrypStorageSTM32_XSPI = {
		.name = "storage-cryp-stm32xspi",
		.init = xspidrv_init,
		.done = xspidrv_done,
